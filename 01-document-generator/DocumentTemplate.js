function createPdfFromDocumentTemplate_(templateId, model, tokens) {
  ...
  const temporaryFile = templateFile.makeCopy('Temporary PDF source - ' + model.reference);
...
function assertDocumentHasTemplateTokens_(containers, tokens) {
  const hasToken = Object.keys(tokens).some(function (key) {
    ...
    return containers.some(function (container) { return Boolean(container.findText(pattern)); });
  });

  if (!hasToken) throw new Error('The Google Doc template does not contain any supported {{TOKEN}} placeholders.');
}
