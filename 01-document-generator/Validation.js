function validateDocumentInput_(input) {
  ...
  if (
    !Object.keys(DOCUMENT_GENERATOR.templateTypes).some(function (key) {
  ...
  if (!model.customerName || model.customerName.length > 120)
    throw new Error("Enter a valid customer name.");
  if (!model.customerAddress || model.customerAddress.length > 500)
    throw new Error("Enter a valid address.");
  if (!model.projectTitle || model.projectTitle.length > 160)
    throw new Error("Enter a valid project title.");
  if (!model.startDate) throw new Error("Enter a valid start date.");
  if (!["GBP", "EUR", "USD"].includes(model.currency))
    throw new Error("Choose a supported currency.");
  if (
    !totalText ||
    !isFinite(model.total) ||
    model.total < 0 ||
    model.total > 999999999.99
  ) {
