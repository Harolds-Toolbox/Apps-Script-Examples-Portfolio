function createPdfFromSpreadsheetTemplate_(config, model, tokens) {
  ...
  const sourceSpreadsheet = SpreadsheetApp.openById(config.spreadsheetTemplateId);
  const sourceSheet = getSpreadsheetTemplateSheet_(sourceSpreadsheet, config.spreadsheetTemplateSheetName);
  const temporarySpreadsheet = SpreadsheetApp.create('Temporary PDF source - ' + model.reference);
  const temporaryFile = DriveApp.getFileById(temporarySpreadsheet.getId());

  try {
    const copiedSheet = sourceSheet.copyTo(temporarySpreadsheet).setName(sourceSheet.getName());
    removeOtherSpreadsheetSheets_(temporarySpreadsheet, copiedSheet.getSheetId());
...
function getSpreadsheetTemplateSheet_(spreadsheet, configuredName) {
  if (configuredName) {
    ...
    if (!configuredSheet) throw new Error('Spreadsheet template tab not found: ' + configuredName);
    return configuredSheet;
  }

  const sheets = spreadsheet.getSheets();
  if (!sheets.length) throw new Error('The spreadsheet template does not contain a sheet.');
  return sheets[0];
}

function assertSheetHasTemplateTokens_(sheet, tokens) {
  const hasToken = Object.keys(tokens).some(function (key) {
    return Boolean(sheet.createTextFinder('{{' + key + '}}').matchCase(true).findNext());
  });

  if (!hasToken) throw new Error('The Google Sheet template does not contain any supported {{TOKEN}} placeholders.');
}
