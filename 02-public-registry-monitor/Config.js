...
const REGISTRY_HEADERS = Object.freeze([
  'Company Number', 'Company Name', 'Status', 'Incorporated On',
  'Address', 'Postcode', 'Activity Codes', 'Registry URL'
]);
...
function registryListProperty_(name) {
  return registryProperty_(name).split(',').map(function (item) { return item.trim(); }).filter(Boolean);
}
