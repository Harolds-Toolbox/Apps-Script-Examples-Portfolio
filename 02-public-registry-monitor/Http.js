function registryFetchJson_(path, query) {
  const url = REGISTRY_CONFIG.baseUrl + path + registryQueryString_(query || {});
  const authorization = 'Basic ' + Utilities.base64Encode(registryProperty_('REGISTRY_API_KEY') + ':');
  let lastError;
  ...
  for (let attempt = 1; attempt <= REGISTRY_CONFIG.maxAttempts; attempt += 1) {
    ...
    Utilities.sleep(Math.min(8000, 600 * Math.pow(2, attempt - 1)) + Math.floor(Math.random() * 300));
  }
...
function registryQueryString_(query) {
  ...
  Object.keys(query).forEach(function (key) {
    ...
    values.forEach(function (value) {
      if (value !== '' && value != null) pairs.push(encodeURIComponent(key) + '=' + encodeURIComponent(String(value)));
    });
  });
