function fetchRegistryCompanies_() {
  ...
  const postcodeAreas = registryListProperty_('REGISTRY_POSTCODE_AREAS').map(function (area) { return area.toUpperCase(); });
  const results = [];
  ...
  return dedupeRegistryCompanies_(results)
    .filter(function (item) { return registryPostcodeMatches_(item.registered_office_address, postcodeAreas); })
...
function registryPostcodeMatches_(address, areas) {
  ...
  const postcode = String(address && address.postal_code || '').trim().toUpperCase();
...
function normalizeRegistryCompany_(item) {
  ...
  return [
    ...
    [address.premises, address.address_line_1, address.locality, address.region].filter(Boolean).join(', '),
    String(address.postal_code || ''),
    Array.isArray(item.sic_codes) ? item.sic_codes.join(', ') : '',
    'https://find-and-update.company-information.service.gov.uk/company/' + encodeURIComponent(String(item.company_number || ''))
  ];
}
