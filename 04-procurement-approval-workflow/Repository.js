...
function findProcurementRequest_(reference) {
  ...
  return { rowNumber: cell.getRow(), request: procurementRowToRequest_(sheet.getRange(cell.getRow(), 1, 1, 13).getValues()[0]) };
}

function saveProcurementRequest_(rowNumber, request) {
  procurementRequestsSheet_().getRange(rowNumber, 1, 1, 13).setValues([procurementRequestToRow_(request)]);
  return request;
}
...
function listProcurementRequests_(email) {
  ...
  return values.slice(1).filter(function (row) {
    return reviewer || String(row[PROCUREMENT_COLUMNS.requesterEmail]).toLowerCase() === email;
  }).map(procurementRowToRequest_).sort(function (left, right) {
...
function procurementRequestToRow_(request) {
  return [request.reference, request.requesterEmail, request.supplier, request.description,
    request.currency, request.amount, request.status, request.informationRequest,
    request.informationResponse, request.createdAt, request.updatedAt,
    request.reviewerEmail, request.version];
}

function procurementRowToRequest_(row) {
  return {
    reference: row[0], requesterEmail: row[1], supplier: row[2], description: row[3],
    currency: row[4], amount: Number(row[5]) || 0, status: row[6],
    informationRequest: row[7], informationResponse: row[8], createdAt: row[9],
    updatedAt: row[10], reviewerEmail: row[11], version: Number(row[12]) || 1
  };
}
