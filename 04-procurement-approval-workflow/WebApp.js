function doGet(event) {
  ...
  if (parameters.mode === "action") {
    ...
    template.payloadBase64 = Utilities.base64EncodeWebSafe(
      JSON.stringify({
        reference: String(parameters.ref || ""),
        action: String(parameters.action || ""),
        version: Number(parameters.version),
        expires: Number(parameters.expires),
        signature: String(parameters.signature || ""),
      }),
    );
    return template
      .evaluate()
      .setTitle("Confirm procurement action")
      .addMetaTag("viewport", "width=device-width, initial-scale=1");
  }
  return HtmlService.createTemplateFromFile("Index")
    .evaluate()
    .setTitle("Procurement workflow")
    .addMetaTag("viewport", "width=device-width, initial-scale=1");
}
...
function submitProcurementRequest(input) {
  const request = createProcurementRequest_(
    validateNewProcurementRequest_(input, procurementUser_()),
  );
  appendProcurementAudit_(
    request.reference,
    "CREATED",
...
function performProcurementAction(input) {
  ...
  if (action === "RESPOND_INFORMATION") {
    ...
    if (!found || String(found.request.requesterEmail).toLowerCase() !== actor)
      throw new Error("Only the requester can submit this response.");
  } else if (!isProcurementReviewer_(actor)) {
    throw new Error("Reviewer access is required.");
  }
  const updated = transitionProcurementRequest_(
    reference,
    action,
    actor,
    input && input.message,
...
function executeSignedProcurementAction(payload) {
  ...
  if (["APPROVE", "REJECT"].indexOf(action) === -1)
    throw new Error("Unsupported signed action.");
  verifyProcurementToken_(
    reference,
    action,
  ...
  const found = findProcurementRequest_(reference);
  if (!found || found.request.version !== Number(value.version))
    throw new Error("This link has already been used or replaced.");
