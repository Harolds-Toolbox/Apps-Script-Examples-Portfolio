function validateNewProcurementRequest_(input, requesterEmail) {
  ...
  if (PROCUREMENT.currencies.indexOf(currency) === -1) throw new Error('Unsupported currency.');
  ...
  return {
    ...
    createdAt: now, updatedAt: now, reviewerEmail: '', version: 1
  };
}
...
function transitionProcurementRequest_(reference, action, actor, message) {
  return withProcurementLock_(function () {
    ...
    if (action === 'REQUEST_INFORMATION') {
      ...
      if (!request.informationRequest) throw new Error('An information request message is required.');
    } else if (action === 'RESPOND_INFORMATION') {
      request.informationResponse = cleanProcurementText_(message, 2000);
      if (!request.informationResponse) throw new Error('A response is required.');
    } else {
      request.reviewerEmail = actor;
    }

    saveProcurementRequest_(found.rowNumber, request);
    appendProcurementAudit_(reference, action, actor, { from: previousStatus, to: next });
    return request;
  });
}
...
function procurementTransitionTarget_(current, action) {
  ...
  if (!target) throw new Error('Action ' + action + ' is not allowed while request is ' + current + '.');
  return target;
}

function cleanProcurementText_(value, maxLength) {
  const text = String(value == null ? '' : value).replace(/\s+/g, ' ').trim();
  if (text.length > maxLength) throw new Error('A submitted value is too long.');
  return text;
}
