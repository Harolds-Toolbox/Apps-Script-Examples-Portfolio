function createProcurementToken_(reference, action, version, expirySeconds) {
  const expires =
    ...
    (expirySeconds || PROCUREMENT.linkLifetimeSeconds);
...
function verifyProcurementToken_(
  reference,
  action,
  version,
  expires,
  ...
  if (!isFinite(numericExpiry) || numericExpiry < Math.floor(Date.now() / 1000))
    throw new Error("This action link has expired.");
  const payload = [
    reference,
    action,
    String(version),
    String(numericExpiry),
  ].join("|");
  const expected = signProcurementPayload_(payload);
  if (!constantTimeProcurementEqual_(expected, String(suppliedSignature || "")))
...
function signProcurementPayload_(payload) {
  ...
  if (secret.length < 32)
    throw new Error(
      "PROCUREMENT_LINK_SECRET must contain at least 32 characters.",
    );
...
function buildProcurementActionUrl_(request, action) {
  const token = createProcurementToken_(
    request.reference,
    action,
    request.version,
  );
  return (
    procurementProperty_("PROCUREMENT_WEB_APP_URL") +
    "?" +
    [
      "mode=action",
