...
function catalogueEndpoints_() {
  let endpoints;
  try { endpoints = JSON.parse(catalogueProperty_('CATALOGUE_ENDPOINTS_JSON')); }
  catch (error) { throw new Error('CATALOGUE_ENDPOINTS_JSON must contain valid JSON.'); }
  if (!Array.isArray(endpoints) || !endpoints.length) throw new Error('At least one endpoint is required.');
  return endpoints.map(function (endpoint) {
    if (!endpoint.name || !endpoint.path) throw new Error('Each endpoint requires name and path.');
    return {
...
function catalogueBook_() {
  return SpreadsheetApp.openById(catalogueProperty_('CATALOGUE_SPREADSHEET_ID'));
}
