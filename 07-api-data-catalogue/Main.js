// Main flow: authenticate → sample endpoints → discover fields → infer links → publish.
function generateApiCatalogue() {
  ...
  try {
    ...
    const relationshipRows = inferCatalogueRelationships_(allFields).map(
      function (relationship) {
        return [
          relationship.endpoint,
          relationship.fieldPath,
          relationship.possibleTarget,
