function getCatalogueAccessToken_() {
  ...
  const response = UrlFetchApp.fetch(
    catalogueProperty_("CATALOGUE_TOKEN_URL"),
    {
      method: "post",
  ...
  if (response.getResponseCode() < 200 || response.getResponseCode() >= 300)
    throw new Error(
      "Token endpoint returned HTTP " + response.getResponseCode(),
    );
  const body = JSON.parse(response.getContentText() || "{}");
  if (!body.access_token)
    throw new Error("Token response did not contain access_token.");
  cache.put(
    "CATALOGUE_OAUTH_TOKEN",
    body.access_token,
    Math.max(60, Math.min(Number(body.expires_in) || 3600, 3600) - 60),
  );
