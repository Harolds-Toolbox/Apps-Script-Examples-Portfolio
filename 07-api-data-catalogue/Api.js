function fetchCatalogueEndpoint_(endpoint) {
  ...
  const url =
    baseUrl +
    (endpoint.path.charAt(0) === "/" ? endpoint.path : "/" + endpoint.path) +
    separator +
    "limit=" +
    endpoint.sampleSize;
  const response = catalogueFetch_(url);
  const selected = endpoint.itemsPath
    ? getCataloguePath_(response, endpoint.itemsPath)
    : response;
  ...
  throw new Error(
    "Endpoint " +
      endpoint.name +
      " did not return an object or array at itemsPath.",
  );
}
...
function catalogueFetch_(url) {
  ...
  for (let attempt = 1; attempt <= CATALOGUE.maxAttempts; attempt += 1) {
    const response = UrlFetchApp.fetch(url, {
      method: "get",
      headers: {
        Authorization: "Bearer " + getCatalogueAccessToken_(),
