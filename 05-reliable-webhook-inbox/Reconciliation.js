function reconcileWebhookChanges() {
  ...
  const checkpoint = properties.getProperty('RECONCILIATION_CHECKPOINT') || new Date(Date.now() - WEBHOOK_CONFIG.reconciliationLookbackMinutes * 60 * 1000).toISOString();
  const startedAt = new Date();
  const response = webhookReconciliationFetch_(baseUrl + (baseUrl.indexOf('?') === -1 ? '?' : '&') + 'since=' + encodeURIComponent(checkpoint));
