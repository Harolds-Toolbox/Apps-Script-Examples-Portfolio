const DASHBOARD_CONFIG = Object.freeze({
  reconciliationSheet: 'Daily Reconciliation',
  countsSheet: 'Operational Counts'
});
...
function getDashboardData() {
  ...
  const book = SpreadsheetApp.openById(requiredDashboardProperty_('DASHBOARD_SPREADSHEET_ID'));
  const businessDate = dashboardDateKey_(new Date());
  const reconciliations = latestDashboardRowsBySite_(
    readDashboardObjects_(book.getSheetByName(DASHBOARD_CONFIG.reconciliationSheet)),
    businessDate
  );
  ...
  const totals = sites.reduce(function (value, site) {
...
function latestDashboardRowsBySite_(rows, dateKey) {
  return rows.reduce(function (latest, row) {
    ...
    if (!latest[site] || timestamp.getTime() > latest[site].timestamp.getTime()) {
      latest[site] = {
        timestamp: timestamp,
...
function latestDashboardCounts_(rows, dateKey) {
  ...
  rows.forEach(function (row) {
    ...
    if (!latest[key] || timestamp.getTime() > latest[key].timestamp.getTime()) {
      latest[key] = { site: site, category: category, timestamp: timestamp, quantity: dashboardNumber_(row.Quantity) };
    }
  });
...
function buildDashboardSites_(reconciliations, counts) {
  const names = {};
  Object.keys(reconciliations).forEach(function (site) { names[site] = true; });
  counts.forEach(function (count) { names[count.site] = true; });
  return Object.keys(names).sort().map(function (site) {
    const reconciliation = reconciliations[site] || { expected: 0, actual: 0, timestamp: null };
    const siteCounts = counts.filter(function (count) { return count.site === site; });
    const quantity = siteCounts.reduce(function (sum, count) { return sum + count.quantity; }, 0);
    return {
      site: site,
      expected: roundDashboardNumber_(reconciliation.expected),
      actual: roundDashboardNumber_(reconciliation.actual),
      variance: roundDashboardNumber_(reconciliation.actual - reconciliation.expected),
      quantity: roundDashboardNumber_(quantity),
      capturedAt: reconciliation.timestamp ? reconciliation.timestamp.toISOString() : '',
      counts: siteCounts.map(function (count) { return { category: count.category, quantity: count.quantity }; })
    };
  });
}
...
function requireDashboardUser_() {
  ...
  const allowed = requiredDashboardProperty_('DASHBOARD_AUTHORIZED_EMAILS').split(',')
    .map(function (value) { return value.trim().toLowerCase(); }).filter(Boolean);
  if (!email) throw new Error('Signed-in identity is unavailable. Check the web-app deployment access setting.');
  if (allowed.indexOf(email) === -1) throw new Error('This account is not authorized to view the dashboard.');
  return email;
}
