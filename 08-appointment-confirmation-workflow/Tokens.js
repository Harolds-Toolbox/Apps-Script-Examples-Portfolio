function issueAppointmentToken_(appointmentId) {
  ...
  const expires = new Date(
    Date.now() + appointmentConfig_().tokenLifetimeHours * 3600000,
  );
...
function consumeAppointmentToken_(token) {
  ...
  try {
    ...
    const values = sheet
      .getRange(2, 1, sheet.getLastRow() - 1, APPOINTMENT.tokenHeaders.length)
      .getValues();
    ...
    if (new Date(row[2]).getTime() < Date.now())
