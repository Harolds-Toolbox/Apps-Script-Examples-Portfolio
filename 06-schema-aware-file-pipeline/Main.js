function runFilePipeline() {
  ...
  try {
    ...
    const routes = filePipelineRoutes_().reduce(function (index, route) { index[route.name] = route; return index; }, {});
    let processed = 0;
    for (let index = 1; index < values.length && processed < FILE_PIPELINE.batchSize; index += 1) {
      ...
      if (status !== 'NEW' && !(status === 'RETRY' && (!retryAt || retryAt <= new Date()))) continue;
...
function discoverPipelineFilesUnlocked_() {
  ...
  const existing = queue.getLastRow() > 1 ? queue.getRange(2, 1, queue.getLastRow() - 1, 1).getDisplayValues()
  ...
  while (files.hasNext()) {
    ...
    if (!existing[file.getId()] && route) rows.push([file.getId(), file.getName(), 'NEW', 0, '', '', route.name, file.getDateCreated(), '', '', '']);
  }
...
function processFileQueueRow_(sheet, rowNumber, row, route) {
  ...
  try {
    ...
    sheet.getRange(rowNumber, 3, 1, 9).setValues([['DONE', attempts, '', '', route.name, row[7], new Date(), schema.hash, schema.rowCount]]);
  } catch (error) {
    const terminal = attempts >= FILE_PIPELINE.maxAttempts;
    const retryAt = terminal ? '' : new Date(Date.now() + FILE_PIPELINE.retryBaseMinutes * Math.pow(2, attempts - 1) * 60 * 1000);
    sheet.getRange(rowNumber, 3, 1, 4).setValues([[terminal ? 'DEAD' : 'RETRY', attempts, retryAt, String(error.message || error).slice(0, 500)]]);
  }
}

function installFilePipeline() {
  ScriptApp.getProjectTriggers().filter(function (trigger) { return trigger.getHandlerFunction() === 'runFilePipeline'; })
