function parsePipelineFile_(fileId, fileName) {
  ...
  if (/\.csv$/i.test(fileName)) return normalizePipelineTable_(Utilities.parseCsv(removePipelineBom_(file.getBlob().getDataAsString())));
...
function parsePipelineXlsx_(file) {
  let convertedId = '';
  try {
    const metadata = { name: 'temporary_pipeline_' + Utilities.getUuid(), mimeType: MimeType.GOOGLE_SHEETS };
    const converted = Drive.Files.create(metadata, file.getBlob(), { fields: 'id' });
...
function normalizePipelineTable_(values) {
  const rows = (values || []).map(function (row) {
    ...
    return copy;
  }).filter(function (row) { return row.some(function (value) { return String(value) !== ''; }); });
