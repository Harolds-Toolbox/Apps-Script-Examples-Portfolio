...
function filePipelineRoutes_() {
  let routes;
  try {
    routes = JSON.parse(filePipelineProperty_("PIPELINE_ROUTES_JSON"));
  } catch (error) {
    throw new Error("PIPELINE_ROUTES_JSON must contain valid JSON.");
  }
  if (!Array.isArray(routes) || !routes.length)
    throw new Error("At least one pipeline route is required.");
  return routes.map(function (route) {
    if (
      !route.name ||
      !route.filePattern ||
      !route.destinationSheet ||
      !Array.isArray(route.requiredHeaders)
    ) {
      throw new Error(
        "Each route requires name, filePattern, destinationSheet and requiredHeaders.",
      );
    }
    return route;
  });
...
function fileQueueSheet_() {
  const sheet = SpreadsheetApp.openById(
    filePipelineProperty_("PIPELINE_CONTROL_SPREADSHEET_ID"),
  ).getSheetByName(FILE_PIPELINE.queueSheet);
