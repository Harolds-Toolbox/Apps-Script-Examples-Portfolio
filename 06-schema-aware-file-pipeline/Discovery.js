...
function findFilePipelineRoute_(fileName, routes) {
  for (let index = 0; index < routes.length; index += 1) {
    if (new RegExp(routes[index].filePattern, "i").test(fileName))
      return routes[index];
  }
  return null;
}
...
function ensureFileQueueHeaders_(sheet) {
  const current = sheet
    .getRange(1, 1, 1, FILE_QUEUE_HEADERS.length)
    .getValues()[0];
  if (current.join("|") !== FILE_QUEUE_HEADERS.join("|"))
    sheet
      .getRange(1, 1, 1, FILE_QUEUE_HEADERS.length)
      .setValues([FILE_QUEUE_HEADERS]);
}
